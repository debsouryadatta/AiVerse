...
import { getVoiceMentorDetailsAction, updateVoiceMentorDetailsAction } from "../actions";
...
export default function Home({ params: { slug } }: Props) {
    ...
    useEffect(() => {
        const updateChatHistory = async (updatedChatHistory: Message[]) => {
            try {
                await updateVoiceMentorDetailsAction(voiceMentorId, session?.data?.user?.id!, { chatHistory: updatedChatHistory });
            } catch (error) {
    ...
    useEffect(() => {
        const getVoiceMentorDetails = async () => {
            setLoading(true);
            try {
                const voiceMentor = await getVoiceMentorDetailsAction(voiceMentorId, session?.data?.user?.id!);
                console.log("VoiceMentor", voiceMentor);
                setVoiceMentorDetails(voiceMentor);

                if (voiceMentor?.chatHistory) {
                    const slicedMessages: Message[] = voiceMentor.chatHistory.slice(-20).map((msg: any) => ({
                        sender: msg.sender || "",
    ...
    const handleSubmit = (e: | React.FormEvent<HTMLFormElement> | React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        ...
        formData.append("voiceMentorDescription", voiceMentorDetails?.description || "");
    ...
    return (
        <div className="h-screen">
            <BeatLoader className="absolute top-1/2 left-1/2 " color="#ffffff" loading={voiceResponseLoading} />
            <div onClick={() => setDisplaySettings(!displaySettings)} className="absolute top-[100px] right-6 z-50 sm:top-16 cursor-pointer">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col bg-black">
                ...
                <div className="fixed bottom-0 w-[100%] md:w-[95%] overflow-hidden bg-black rounded-t-3xl">
                    <Recorder uploadAudio={uploadAudio} recordingStatus={recordingStatus} setRecordingStatus={setRecordingStatus} isTimerRunning={isTimerRunning} />
                    <div className="">
                        <VoiceSynthesizer state={state} displaySettings={displaySettings} recordingStatus={recordingStatus} setRecordingStatus={setRecordingStatus} voiceMentorDetails={voiceMentorDetails!} isTimerRunning={isTimerRunning} setIsTimerRunning={setIsTimerRunning} setMessages={setMessages} />
                    </div>
                </div>
            </form>
        </div>
